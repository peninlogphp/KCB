...
public class WithdrawCashHandler extends AsyncHttpResponseHandler{

	LipukaApplication lipukaApplication;
	Activity activity;
	String[] names;
    int[] ids;
   public WithdrawCashHandler(LipukaApplication lipukaApplication,
		   Activity activity){
	   super();
     ...
     if (activity == lipukaApplication.getCurrentActivity()){
        ...
        if (response.getInt("STATUS_CODE") == 1){
            ...
            if (activity == null || !lipukaApplication.isActivityVisible(activity.getClass())){
        		lipukaApplication.showGenericNotification(response.getString("STATUS_MESSAGE"), EcobankMain.class, R.id.notification_response);
return;
            }
	...
	if (activity == null || !lipukaApplication.isActivityVisible(activity.getClass())){
		lipukaApplication.showGenericNotification(errorMsg, activity.getClass(), R.id.notification_response);
    	return;
    }
