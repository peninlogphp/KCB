...
public class ManageTransactionsDetails extends Activity implements OnClickListener, ResponseActivity,
DateCaptureActivity{
	   
	Button submit, cancel;
	EditText fname;
	EditText lname, testQuestion, testAnswer;
	RelativeLayout help;
...
try{
	        setContentView(R.layout.wu_manage_transactions_details);
	        TextView title = (TextView) findViewById(R.id.title);
	        title.setText("Manage Transaction");
	        ...
	        testQuestion = (EditText)findViewById(R.id.test_question_field);
	        testAnswer = (EditText)findViewById(R.id.test_answer_field);
	        try {
		        transaction = new JSONObject(transactionStr);
	    ...
	    protected void onStart() {
			...
			lipukaApplication.setCurrentActivity(this);
			lipukaApplication.setActivityState(ManageTransactionsDetails.class, true);
			}
	   
	    @Override
	    protected void onStop() {
	        super.onStop();
			lipukaApplication.setActivityState(ManageTransactionsDetails.class, false);
	    }
	    @Override
	    		...
	    		if (submit == arg0){
	    		    ...
	    		    String lnameStr = lname.getText().toString();
	    		        			...
	    		        			if(valid){
        						    	...
        						    	try{
        						    		lipukaApplication.putPayload("send_money_txn_id", transaction.getString("id"));
        						    		lipukaApplication.putPayload("first_name", fnameStr);
        						    		lipukaApplication.putPayload("last_name", lnameStr);
        						    		lipukaApplication.putPayload("test_question", testQuestion.getText().toString());
        						    		lipukaApplication.putPayload("test_answer", testAnswer.getText().toString());
        					    			lipukaApplication.setServiceID("37");
        				    			    }
        				    			    catch (org.json.JSONException jsonError) {
        				    				     Log.d(Main.TAG, "jsonError: ", jsonError);
        				    			      }  
	    		    	...
	    		    	this.amountStr = "You have successfully edited your transaction. Thank you.";
	    			...
	    			lipukaApplication.setCurrentDialogTitle("PIN");
		    	      lipukaApplication.setCurrentDialogMsg("Please enter your mobile banking PIN");
		    	showDialog(Main.DIALOG_PIN_ID);  
		    	this.amountStr = "Your cancellation/refund request has been accepted. You will be contacted within 24 hours by our customer service to complete your request. Thank you.";
