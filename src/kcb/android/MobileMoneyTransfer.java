...
public class MobileMoneyTransfer extends Activity implements OnClickListener, ResponseActivity{
...
try{
	        setContentView(R.layout.mobile_money_transfer);
	        TextView title = (TextView) findViewById(R.id.title);
	        title.setText("Mobile Money Transfer");
	  ...
	  try{      JSONArray sources = new JSONArray(lipukaApplication.loadSourceAcounts());
	      ...
	      currentSource.getString("name"), currentSource.getString("value"));
	       ...
	       final Spinner providerSpinner = (Spinner) findViewById(R.id.provider_spinner);
	       providerSpinner.setOnItemSelectedListener(new OnProviderSelectedListener());
	        
	  try{      JSONArray sources = new JSONArray(lipukaApplication.loadMobileMoneyProviders());
	         providersArray = new LipukaListItem[sources.length()];
	      JSONObject currentSource;
	        for(int i = 0; i < sources.length(); i++){
	        	currentSource = sources.getJSONObject(i);
	        	LipukaListItem lipukaListItem = new LipukaListItem("", 
	      currentSource.getString("name"), currentSource.getString("value"));
	        	providersArray[i]= lipukaListItem;   	
	        }
	        }catch(Exception ex){
		    	Log.d(Main.TAG, "creating providers list error", ex);
	
	    	}
	   	 ComboBoxAdapter providerAdapter = new ComboBoxAdapter(this, android.R.layout.simple_spinner_item, providersArray);
	   	...
	   	Button helpButton = (Button)findViewById(R.id.help);
	    	...
	    	Log.d(Main.TAG, "creating mobile money transfer error", ex);
	    ...
	    protected void onStart() {
	        super.onStart();
			lipukaApplication.setCurrentActivity(this);
			lipukaApplication.setActivityState(MobileMoneyTransfer.class, true);
			}
	   
	    @Override
	    protected void onStop() {
	        super.onStop();
			lipukaApplication.setActivityState(MobileMoneyTransfer.class, false);
	    }
	    @Override
	    			    			...
	    			    			payloadBuffer.append(amountStr+"|");	    				
	    			    				...
	    			    				/*lipukaApplication.setCurrentDialogTitle("Confirm");
	    			    			      lipukaApplication.setCurrentDialogMsg("You are about to transfer KES "+amountStr+
	    			    			    		  " to your "+selectedProvider+" account. Press \"OK\" to transfer now or \"Cancel\" to edit transfer details");
	    			    			showDialog(Main.DIALOG_CONFIRM_ID);*/
	    			    		           showResponse();
	    			    			this.amountStr = amountStr;
destinationStr  = selectedProvider;
	    			    			}else{
	    	...
	    	pb.setVisibility(View.VISIBLE);
	    		...
	    		public class OnProviderSelectedListener implements OnItemSelectedListener {

		 	        public void onItemSelected(AdapterView<?> parent,
		 	            View view, int pos, long id) {
		 	        	selectedProvider = providersArray[pos].getText();
		 	        	}
		 	        public void onNothingSelected(AdapterView parent) {
		 	          // Do nothing.
		 	        }
	   ...
	   public void showResponse(){
			lipukaApplication.setCurrentDialogTitle("Response");
		     /* lipukaApplication.setCurrentDialogMsg("Dear Alice, you have successfully transferred KES "+amountStr+" to your "+destinationStr+" account. Thank you");
