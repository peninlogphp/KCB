...
import lipuka.android.model.responsehandlers.FetchCardNumbersHandler;
...
import lipuka.android.view.adapter.ManageSavedCardNumbersAdapter;
...
public class ManageSavedCardNumbers extends Activity implements OnClickListener, ResponseActivity,
...
try{
	        setContentView(R.layout.saved_card_numbers);
	    ...
	    protected void onStart() {
			...
			lipukaApplication.setCurrentActivity(this);
			lipukaApplication.setActivityState(ManageSavedCardNumbers.class, true);	    
	    }
	   
	    @Override
	    protected void onStop() {
	        super.onStop();
			lipukaApplication.setActivityState(ManageSavedCardNumbers.class, false);
	    }
	    @Override
	    		...
	    		if (submit == arg0){
	    			   lipukaApplication.clearPayloadObject();
		    			...
		    			if(cardNoStr.length() != 16){
		    				valid = false;
		    				errorBuffer.append("Please enter a 16 digits card number\n");
		    			}	
		    			...
		    			if(valid){
		    		        			...
		    		        			/*	payloadBuffer.append(enteredStartDate.get(Calendar.YEAR));
		    				    			...
		    				    			lipukaApplication.setServiceID(String.valueOf(Constants.ADD_CARD_NUMBER));

		    		        				lipukaApplication.setCurrentDialogTitle("PIN");
		    		    		    	      lipukaApplication.setCurrentDialogMsg("Please enter your mobile PIN");
    		    	...
    		    	Intent intent = new Intent(this, EditSavedCardNumber.class);        
...
lipukaApplication.clearPayloadObject();
		    	try{
		    		lipukaApplication.putPayload("saved_destination_beneficiary_id", currentCard.getString("saved_card_beneficiary_id"));
    				deletedCardID = currentCard.getString("saved_card_beneficiary_id");
	    			lipukaApplication.setServiceID(String.valueOf(Constants.DELETE_CARD_NUMBER));
...
if(!fetchedCards){
    lipukaApplication.clearPayloadObject();
	lipukaApplication.consumeService("86", new FetchCardNumbersHandler(lipukaApplication, this));
}else{
	//myAccount.setVisibility(View.VISIBLE);
  ...
  try{      
   		...
   		ManageSavedCardNumbersAdapter adapter = new ManageSavedCardNumbersAdapter(this, items);
...
showOrHideEditCard.setCompoundDrawablesWithIntrinsicBounds( img, null, null, null );	
	...
	if(deletedCardID.equals(card.getString("saved_card_beneficiary_id"))){
		cardToDelete = card;
			        	}	
		           }
		   		 }catch(Exception ex){
